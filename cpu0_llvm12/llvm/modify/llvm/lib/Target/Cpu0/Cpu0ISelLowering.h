...
// selection DAG.
//
// 此文件定义了Cpu0用于将LLVM代码降低到选择DAG的接口。
//
//===----------------------------------------------------------------------===//
...
namespace llvm {
  ...
  public:
    ...
    //  DAG node.
    //  getTargetNodeName - 此方法返回特定目标DAG节点的名称。
    const char *getTargetNodeName(unsigned Opcode) const override;

  protected:

    /// ByValArgInfo - Byval argument information.
    //  ByValArgInfo - Byval参数信息。
    struct ByValArgInfo {
  ...
  protected:
    ...
    // 从TargetMachine缓存ABI，我们在任何地方都使用它。
