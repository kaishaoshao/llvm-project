...
Cpu0Subtarget::Cpu0Subtarget(const Triple &TT, const std::string &CPU,
                             const std::string &FS, const Cpu0TargetMachine &_TM):
    Cpu0GenSubtargetInfo(TT, CPU,/* TuneCPU */ FS),
    IsLittle(little), TM(_TM), TargetTriple(TT),TSInfo(),
    InstrInfo(
        Cpu0eInstrInfo::create(initializeSubtargetDependencies(CPU, FS, TM))),
...
Cpu0Subtarget &Cpu0Subtarget::initializeSubtargetDependencied(StringRef CPU, 
                                                              StringRef FS,
